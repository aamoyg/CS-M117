...
//struct to hold arguments: t, tracker_task
typedef struct task_downargs {
	task_t *t;
	task_t *tracker_task;
}task_downargs_t;
...
static size_t read_tracker_response(task_t *t)
{
	...
	while (1) {
		...
		else if (ret == TBUF_END)
		{
			...
			if (headpos == tailpos && t->head != t->tail)
				return TASKBUFSIZ;
			else
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	char temp[TASKBUFSIZ*2] = "";
	size_t start = TASKBUFSIZ;
	s1 = temp + start;
	unsigned headpos;
	unsigned tailpos;
	//do this loop while buffer is full
	do
	{
		start = TASKBUFSIZ;
		//read_tracker_response
		read_tracker_response(tracker_task);
		
		headpos = (tracker_task->head % TASKBUFSIZ);
		tailpos = (tracker_task->tail % TASKBUFSIZ);
		//copy tracker buf into temp buf middle
		memcpy(temp+TASKBUFSIZ, tracker_task->buf, TASKBUFSIZ);
		//read from temp buf and parse
		while (1)
		{
			s2 = memchr(s1, '\n', TASKBUFSIZ*2 + temp - s1);
			//incomplete line
			if (!(p = parse_peer(s1, s2 - s1)))
			{
				if((headpos == tailpos && tracker_task->head != tracker_task->tail))
				{
					start = s1 - temp - TASKBUFSIZ;
					//move left over ending to before the middle 
					memcpy((temp + start), s1, (TASKBUFSIZ*2 + temp - s1));
				}
				break;
			}
			p->next = t->peer_list;
		...
		if((headpos == tailpos && tracker_task->head != tracker_task->tail))
			s1 = temp + start;
	} while ((headpos == tailpos && tracker_task->head != tracker_task->tail));
	if (s1[0] != '2')
	{
...
static void task_download(void* taskv)
{
	//cast argument into task_downargs_t*
	task_downargs_t* task = (task_downargs_t *) taskv;

	int i, ret = -1;
	assert((!task->t || task->t->type == TASK_DOWNLOAD)
	       && task->tracker_task->type == TASK_TRACKER);

	// Quit if no peers, and skip this peer
	if (!task->t || !task->t->peer_list) {
		error("* No peers are willing to serve '%s'\n",
		      (task->t ? task->t->filename : "that file"));
		task_free(task->t);
		return;
	} else if (task->t->peer_list->addr.s_addr == listen_addr.s_addr
		   && task->t->peer_list->port == listen_port)
		goto try_again;

	// Connect to the peer and write the GET command
	message("* Connecting to %s:%d to download '%s'\n",
		inet_ntoa(task->t->peer_list->addr), task->t->peer_list->port,
		task->t->filename);
	task->t->peer_fd = open_socket(task->t->peer_list->addr, task->t->peer_list->port);
	if (task->t->peer_fd == -1) {
	...
	//evil buffer overflow
	if (!evil_mode || evil_mode == 2)
		osp2p_writef(task->t->peer_fd, "GET %s OSP2P\n", task->t->filename);
	else if (evil_mode == 1)
	{
		char evil[500];
		int i = 0;
	...
	for (i = 0; i < 50; i++) {
		if (i == 0)
			strcpy(task->t->disk_filename, task->t->filename);
		else
		{
			if (FILENAMESIZ > strlen(task->t->filename) + 4)
				sprintf(task->t->disk_filename, "%s~%d~", task->t->filename, i);
			else
				return;
		}
		task->t->disk_fd = open(task->t->disk_filename,
				  O_WRONLY | O_CREAT | O_EXCL, 0666);
		if (task->t->disk_fd == -1 && errno != EEXIST) {
			error("* Cannot open local file");
			goto try_again;
		} else if (task->t->disk_fd != -1) {
			message("* Saving result to '%s'\n", task->t->disk_filename);
			break;
		}
	}
	if (task->t->disk_fd == -1) {
		error("* Too many local files like '%s' exist already.\n\
* Try 'rm %s.~*~' to remove them.\n", task->t->filename, task->t->filename);
		task_free(task->t);
		return;
	}
	...
	int count = 0;
	while (1) {
		int ret = read_to_taskbuf(task->t->peer_fd, task->t);
		if (ret == TBUF_ERROR) {
			error("* Peer read error");
			goto try_again;
		} else if (ret == TBUF_END && task->t->head == task->t->tail)
			/* End of file */
			break;

		ret = write_from_taskbuf(task->t->disk_fd, task->t);
		if (ret == TBUF_ERROR) {
	...
	if (task->t->total_written > 0) {
		message("* Downloaded '%s' was %lu bytes long\n",
			task->t->disk_filename, (unsigned long) task->t->total_written);
		// Inform the tracker that we now have the file,
		// and can serve it to others!  (But ignore tracker errors.)
		if (strcmp(task->t->filename, task->t->disk_filename) == 0) {
			pthread_mutex_lock (&mutextask);
			osp2p_writef(task->tracker_task->peer_fd, "HAVE %s\n",
				     task->t->filename);
			(void) read_tracker_response(task->tracker_task);
			pthread_mutex_unlock (&mutextask);
		}
		task_free(task->t);
		return;
    ...
    try_again:
	if (task->t->disk_filename[0])
		unlink(task->t->disk_filename);
	// recursive call
	task_pop_peer(task->t);
	task_download(taskv);
...
static void task_upload(task_t *t)
{
	...
	//if string is too long to fit into filename
	if (t->tail > 255 + strlen("GET  OSP2P\n"))
		goto exit;
	...
	//check if filename contains '/' for files in different directories.
	if(memchr(t->filename, '/', strlen(t->filename)))
		goto exit;
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	task_downargs_t* task = (task_downargs_t*) malloc(sizeof(task_downargs_t)*argc);	

	int size = argc;
	// First, download files named on command line. using pthreads
	for (; argc > 1; argc--, argv++)
	{
		if ((task[argc-1].t = start_download(tracker_task, argv[1])))
		{
			task[argc-1].tracker_task = tracker_task;
			pthread_create(&threads[argc-1], &attr, (void *) &task_download, (void *) &task[argc-1]);
		}
	}
	...
	for (; size > 1; size--)
	{
		pthread_join(threads[size-1], &status);
	}
