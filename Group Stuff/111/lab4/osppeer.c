...
const int UPLOAD = 2;
const int DOWNLOAD_TIMEOUT = 20;
const int UPLOAD_TIMEOUT = 20;
...
const int NUM_ATTACK_MODES = 5;
const int NUM_DOS_ATTEMPTS = 100;

// different types of accacks
const int ACCESS_FILES = 1;
const int ATTACK_UPLOAD_TIMEOUT = 2;
const int DOS = 3;
const int CORRUPT_DATA = 4;
const int BUFFER_OVERFLOW = 5;
...
#define TASKBUFSIZ	40969	// Size of task_t::buf
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	// change to strncpy incase the filename is not null terminated and prevent buffer overflow
...
*
*
Need to edit task_download since using it as a function that each thread will run concurrently
...
static void task_download(void* thread_data)
{
	thread_info_t* thread_info = (thread_info_t*) thread_data;		// cast it into the thread_info that is passed in
	
	task_t *t = thread_info->t;
	task_t *tracker_task = thread_info->tracker_task;
	
	// lock thread mutex
	if(thread_info->recursive == 0)
	{
		pthread_mutex_lock(&thread_info->mutex);
		thread_info->started = 1;
	}
	...
	// If in evil mode, attempt to download file from outside of peer's working directory.
	
	char filename[FILENAMESIZ];
	if (evil_mode == ACCESS_FILES)
		strncpy(filename, "/etc/hosts", FILENAMESIZ);
	else
		strncpy(filename, t->filename, FILENAMESIZ);
	filename[FILENAMESIZ-1] = '\0';	

	// Attempt DoS attack
	int attempts = (evil_mode == DOS) ? NUM_DOS_ATTEMPTS : 1;
	pthread_mutex_lock (&tracker_mutex);

	 // exploit buffer overflow on remote peer by requesting file with filename length greater than FILENAMESIZ
    char big_filename[FILENAMESIZ * 2];
    strncpy(big_filename, filename, FILENAMESIZ);
    if (evil_mode == BUFFER_OVERFLOW)
	{	
		int i;
		for (i = strlen(filename); i + strlen(filename) < FILENAMESIZ * 2; i += strlen(filename))
			strcpy(big_filename + i, filename);    //repeatedly copy filename into oversized buffer
	message("WANT %s\n", big_filename);
	}
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	if (t->disk_filename[0])
		unlink(t->disk_filename);
	// recursive call
	thread_info->recursive = 1;
	task_pop_peer(t);
	task_download(thread_data);
	
    exit:

	thread_info->exited = 1;
	pthread_mutex_unlock(&thread_info->mutex);
	pthread_cond_signal(&thread_info->condition);
	task_free(t);
	pthread_exit(NULL);
}
...
/*
*
*Need to edit task_upload since using it as a function that each thread will run concurrently
...
static void task_upload(void *thread_data_void)
{

	thread_info_t* thread_info = (thread_info_t*) thread_data_void;
	...
	// If length of file name is larger than the max file name size, exit
	if (strlen(t->filename) >= FILENAMESIZ)
		goto exit;
	
	int i;

	// checking to see if it attempts to leave the current directory, and exit if it does
	for (i = 0; i < FILENAMESIZ && t->filename[i] != '\0'; i++)
	{
		if (t->filename[i] == '/')
		{
			error("* Invalid filename: %s\n", strlen(t->filename), t->filename);
			goto exit;
		}
	}
	...
	if (evil_mode == ATTACK_UPLOAD_TIMEOUT)
		goto exit;
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	{
		if (thread_timeout(node1, DOWNLOAD_TIMEOUT, DOWNLOAD) == RESTART)
		{
	...
	// Accept connections from peers and upload files
	while ((t = task_listen(listen_task)))
	{
		//task_upload(t);
		node1 = list_add(uploads, t, NULL);		// add the upload task to the upload list
		if(!node1)
		{
		...
		node1->thread_info->start_time = time(NULL);
		status = pthread_create(node1->thread, &attr, (void*) &task_upload, (void*) node1->thread_info);	// create a thread for the upload task
	...
	// finish uploads
	for(node1 = uploads->head->next /*start at first node*/; node1 != uploads->head; node1 = node1->next)
	{
		thread_timeout(node1, UPLOAD_TIMEOUT, UPLOAD);
	}
	message("* Finished all upload requests\n");
	...
	// destroy pthread information
	pthread_attr_destroy(&attr);
	pthread_mutex_destroy(&taskbuf_mutex);
...
int thread_timeout(node_t* node, const int timeout_delay, const int type)
{
	...
	if(timeout_delay != UPLOAD_TIMEOUT && timeout_delay != DOWNLOAD_TIMEOUT)
